...
These are plots that are identical (or near-identical) across all tasks:
...
def plot_training_curves(
    ...
    fig, axs = plt.subplots(
        ...
        squeeze=False,  # always 2D, even when n_metrics==1
    )
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(figures_dir, 'training.svg'), format='png')
    plt.close(fig)
...
def log_firing_rate_stats(state, sample_batch, logger: logging.Logger) -> None:
    ...
    # learning_utils.compute_firing_rate returns spikes/ms; multiply for Hz.
