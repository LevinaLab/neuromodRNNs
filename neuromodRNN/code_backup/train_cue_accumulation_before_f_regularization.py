...
from src.e_prop import plots, e_prop_updates, tasks
...
def model_from_config(cfg)-> LSSN:
  ...
  feedback_key,local_connectivity_key = random.randint(subkey, (2,),10000, 10000000)
  ...
  model = LSSN(n_ALIF=cfg.net_arch.n_ALIF,
              ...
              local_connectivity_key= local_connectivity_key,
...
def train_step(
    ...
    # time steps
    grads = e_prop_updates.e_prop_grads(eligibility_inputs, state, LS_avail, local_connectivity)
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(train_batches):
        state, metrics = train_step_fn(state, batch, LS_avail, local_connectivity)
