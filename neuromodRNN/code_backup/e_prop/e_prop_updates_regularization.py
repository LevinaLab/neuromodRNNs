...
def low_pass_eligibility_trace(low_pass_trace, eligibility_trace, eligibility_params):
...
def batched_low_pass_eligibility_trace(low_pass_trace, eligibility_trace, eligibility_params):
...
def learning_signal(y: Array, true_y: Array, kernel: Array) -> Array:
    ...
    # in case true y has shape (n_t,) expand dim to have n_out =1 (important for tasks with single readout cell)

    # For classification tasks, should pass y as being already softmax result  
...
def vectorized_grads(batch_init_eligibility_carries:Dict[str,Array], batch_inputs: Tuple[Array, Array,Tuple[Array, Array, Array]], eligibility_params: Dict[str,Dict], LS_avail: float=0) -> Tuple[Array, Array]:
    """
    ...
    L = batched_learning_signal(y_batch[:,-LS_avail:,:],true_y_batch[:,-LS_avail:,:],kernel) #n_batch, n_t, n_rec 
    ...
    _, eligibility_traces = lax.scan(
        lambda carry, input: batched_eligibitity_trace(carry, input, eligibility_params),
    ...
    _, low_eligibility_traces = lax.scan(
        lambda carry, input:batched_low_pass_eligibility_trace(carry, input, eligibility_params),
        batch_init_eligibility_carries['a_eligibility_vector'], # ugly, but has the same shape as the init low_eligibility_trace would have and also starts as 0
        eligibility_traces
    )
    ...
    update = jnp.einsum('btri,tbri->ir', jnp.expand_dims(L,axis=3), crop_low_trace)
    return eligibility_traces, update

###################################################################################################

#Firing Rate regularization
###################################################################################################
def firing_rate_regularization(z, eligibility_traces,trial_length, f_target, c_reg):
    ...
    firing_rates = utils.compute_firing_rate(z, trial_length) #(n_batches, n_rec)
    error = (f_target /1000) - firing_rates # (n_batches, n_rec) divide by 1000, because all units are in ms, but f_target should be passed in Hz
    acum_eligibility_trace = jnp.sum(eligibility_traces, axis=0) # n_batches,n_post, n_pre
    
    reg_update = (c_reg / trial_length[:,None,None]) * error[:,:, None] * acum_eligibility_trace # n_post, n_pre
    reg_update = jnp.mean(reg_update, axis=0)
    return jnp.transpose(reg_update, (1,0)) # grads must be n_pre, n_post, because of shape of weights
...
def output_grads(init_batch_trace: Dict[str,Array], batch_inputs: Tuple[Array, Array, Array], params: Dict[str,Dict], LS_avail:float=0) ->Tuple[Array, Array]:
    """
    ...
    jax.debug.print("shape of eligibility signal{}", crop_trace.shape)
    return traces, grads




def e_prop_grads(eligibility_inputs: Tuple[Array, Array, Array, Array], state,LS_avail: int, local_connectivity: bool, f_target, c_reg) ->Dict[str, Dict]:
    """
    ...
    firing_reg_grad = firing_rate_regularization(z, rec_traces,trial_length, f_target, c_reg)
    
    grads['ALIFCell_0']['recurrent_weights'] +=  firing_reg_grad
